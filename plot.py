...
def plotGraph(tickerCode):
    alpha_api_key = '59DS0OI9280PDLHV'
    plotly_api_key = 'm8QrpgwPTs75sYVRmwNJ'
    plotly.tools.set_credentials_file(username='xuanmingt', api_key= plotly_api_key)

    # GET Alphavantage daily stock data in CSV format
    # Using panda library to read it in CSV format
    df = pd.read_csv("https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol="+ tickerCode +"&apikey="+ alpha_api_key +"&datatype=csv")
